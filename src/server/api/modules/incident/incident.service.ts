...
Imagini: ${imagesCount} fișiere atașate
      ...
      */
      ...
      if (incident.receiveIncidentUpdates && user.email) {
        const userHtml = `
<!DOCTYPE html>
<html lang="ro">
<head>
  ...
  <div style="max-width:600px;margin:24px auto;background:#fff;border-radius:16px;box-shadow:0 2px 8px rgba(0,0,0,0.04);font-family:'Poppins',Arial,sans-serif;overflow:hidden;">
    ...
    <div style="padding:32px;">
      ...
      <div style="margin-bottom:24px;">
        ...
        <ul style="padding-left:18px;margin:0;list-style-type:none;">
          <li><strong>Adresă:</strong> ${incident.address ?? "Nespecificată"}</li>
          ${
            incident.latitude && incident.longitude
              ? `<li><a href="https://www.google.com/maps?q=${incident.latitude},${incident.longitude}" style="color:oklch(84.42% 0.172 84.93);text-decoration:underline;">🗺️ Vezi locația pe Google Maps</a></li>`
              : ""
          }
          <li><strong>Imagini trimise:</strong> ${
            incident.imageKeys && incident.imageKeys.length > 0
              ? `${incident.imageKeys.length} imagine${incident.imageKeys.length > 1 ? "i" : ""} atașat${incident.imageKeys.length > 1 ? "e" : "ă"}`
              : "Nicio imagine atașată"
          }</li>
        </ul>
      </div>
...
Rezumat incident:
- Adresă: ${incident.address ?? "Nespecificată"}
- Coordonate: ${latitude ?? "N/A"}, ${longitude ?? "N/A"}
